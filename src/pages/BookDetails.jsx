...
import { ArrowLeft, Star, ShoppingCart, BookOpen, Clock, Calendar, Globe } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchBook = async () => {
      ...
      try {
        ...
        const price = s?.listPrice?.amount || (Math.floor(Math.random() * 16) + 9) + 0.99;
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-0 relative z-10">
            ...
            <div className="md:col-span-8 lg:col-span-8 xl:col-span-9 p-8 md:p-12 flex flex-col">
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-12 pb-12 border-b border-gray-100 dark:border-gray-800">
                 <button 
                   onClick={() => addToCart(book)}
                   className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-10 py-4 bg-primary hover:bg-primary-dark text-white rounded-xl font-bold text-lg transition-all shadow-xl shadow-primary/25 hover:-translate-y-1 active:translate-y-0"
                 >
                   <ShoppingCart className="w-5 h-5 -ml-1" /> Add to Cart
                 </button>
              </div>
