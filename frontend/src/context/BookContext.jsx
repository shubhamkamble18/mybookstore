...
const formatBook = (item) => {
  ...
  const defaultPrice = Math.floor(Math.random() * (999 - 100 + 1)) + 100;
  const apiPrice = s?.listPrice?.amount;
  // Clamp all prices between ₹100 and ₹9,999
  const price = apiPrice ? Math.max(100, Math.min(Math.round(apiPrice * 85), 9999)) : defaultPrice;
...
const formatLocalBook = (book) => ({
  id: `local-${book.id}`,
  dbId: book.id,
  title: book.title,
  author: book.author,
  price: Math.max(100, Math.min(Math.round(book.price * 85), 9999)),
  image: book.image_url,
  highResImage: book.image_url,
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const fetchLocal = backend.get('/api/products/').catch(() => ({ data: [] }));

        const [carouselData, bestsellersData, newArrivalsData, localData] = await Promise.all([
          fetchCarousel, fetchBestsellersReq, fetchNewReq, fetchLocal
        ]);

        const formattedLocal = (localData.data || []).map(formatLocalBook);

        if (carouselData.items) setCarouselBooks(carouselData.items.map(formatBook));

        // Merge local products into bestsellers and arrivals for visibility
        let combinedBestsellers = formattedLocal;
        if (bestsellersData.items) combinedBestsellers = [...combinedBestsellers, ...bestsellersData.items.map(formatBook)];
