...
const BookCard = ({ id, title, author, price, image, highResImage, rating, isLocal, variant = 'default' }) => {
  const { addToCart } = useCartContext();
  const [imgSrc, setImgSrc] = useState(image || '/placeholder-book.png');
  const [isHovered, setIsHovered] = useState(false);
  const [isLoaded, setIsLoaded] = useState(false);

  // Effect to handle high-res image loading or fallback
  useEffect(() => {
    if (highResImage) {
      const img = new Image();
      img.src = highResImage;
      img.onload = () => setImgSrc(highResImage);
      img.onerror = () => setImgSrc(image || '/placeholder-book.png');
    } else {
      setImgSrc(image || '/placeholder-book.png');
    }
  }, [highResImage, image]);
    ...
    <div className={containerClasses} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      ...
      <Link to={`/book/${id}`} className="relative aspect-[2/3] overflow-hidden bg-gray-50 dark:bg-gray-900 flex items-center justify-center p-6 cursor-pointer block">
        ...
        {isLocal && (
          <div className="absolute top-3 left-3 z-10">
            <span className="px-2 py-1 bg-amber-500 text-white text-[10px] font-bold uppercase tracking-wider rounded-md shadow-sm ring-1 ring-amber-400">
              Local Stock
            </span>
          </div>
        )}
        ...
        {imgSrc ? (
          <img 
            ...
            className={`w-full h-full object-cover rounded shadow-md transition-all duration-700 ${isLoaded ? 'opacity-100' : 'opacity-0'} ${isHovered ? 'scale-105' : 'scale-100'}`}
          />
        ) : (
          <div className="w-full h-full bg-gray-200 dark:bg-gray-700 rounded shadow-md flex flex-col items-center justify-center text-gray-400 p-4">
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-grow">
        ...
        <div className="mt-auto flex items-center justify-between">
          <span className="text-lg font-bold text-gray-900 dark:text-white">{toINR(price)}</span>
          <button 
